...
A PARA-style Obsidian vault for the Reservoir project.

## How To Operate Here

### When asked to update the spec
- The canonical spec lives in `10_Projects/Reservoir/Specs/` (if a spec exists). It is the source of truth.
...
- **A risk** → create a note in `10_Projects/Reservoir/Risks/` using `_Templates/Risk.md`. Frontmatter `type: risk` is required for the Risks Base view.
- **An open question** → `10_Projects/Reservoir/Open Questions/` using `_Templates/Open Question.md`. Frontmatter `type: open-question`.
